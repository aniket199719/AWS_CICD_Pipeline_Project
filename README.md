...
- Under IAM select the user created above followed by the creation of Access key for Command Line Interface (CLI) and download the csv file

- Search for ECR service, Create a new repo keeping it private, give the name for URL, copy the URL and save it

- Search EC2 Instance service, launch instance, give the name for instance, select ubuntu, and select the tier as per the requirement, allow http and https traffic now launch the instance (DELETE IT WHEN DONE Charges might incured)

- For the above created intance goto Security -> Select the security group -> Click on edit inbound rules -> Add rule as "Custom TCP" and Port range = the port mentioned in "app.py" file

- Select the instance id for the above created instance and click on connect 

- Execute following steps in Command Prompt on AWS:
...
- Check the status of runner by, Going to settings -> Actions -> Runners -> Under status there should be green dot saying idle
...
- click on New repository secret -> name it as "ECR_REPO_NAME" = "studentperformance" and give the same name found in the end of previously saved URL (ex: "211125765812.dkr.ecr.us-east-2.amazonaws.com/studentperformance")	
